...
All data and code needed to fully reproduce replicate our study this manuscript are publicly available at from github.com/DanOvando/population-effects-of-mpas

All analysis were performed in `r R.version.string`. Package versions are shown in Table.\@ref(tab:pkg-info).
...
knitr::kable(equations,
             escape = FALSE,
             caption = "Distributions of variables used in MPA simulations. Simulations combine random combinations of the variables included here, and using those sampled states simulate a scenario with and a scenario without MPAs. The difference between these two scenarios is then used to calcualte the effect of the MPA in that simulated scenario.",
             format = "pipe",
             align = "l") %>% 
...
Table.1 - Range of simulated variables
...
The list of species included in our base results is available in Table.\@ref(tab:fish-tab). 

```{r fish-tab}
a = life_history_data %>% 
  filter(classcode %in% unique(used_data$classcode)) %>% 
  select(taxa, commonname, targeted) %>% 
  mutate(targeted = case_when(targeted == 1 ~ "Targeted",
                              TRUE ~ "Non-Targeted")) %>% 
  rename("Scientific Name" = taxa,
         "Common Name" = commonname,
         "Fishery Status" = targeted) %>% 
  arrange(`Fishery Status`)

  knitr::kable(
    a,
    digits = 2,
    caption = "Fin-fish species available in PISCO data used in primary difference-in-difference regression.",
    longtable = TRUE,
    booktabs = TRUE ) %>%
  kableExtra::kable_styling(latex_options = c("hold_position", "repeat_header"))
```
...
did_fit %>%
  broom.mixed::tidy(conf.int = TRUE, prob = 0.89) %>%
    rename(lower = conf.low,
         upper = conf.high) %>% 
  select(term, estimate, lower, upper) %>%
  rename(
...
We also ran a frequentist version of our results, as further demonstration that our results are not driven by our priors or Bayesian framework in general. For this version we applied a Newey-West correction to the standard errors to account for the clustering of the observations in space and time (Fig.\@ref(fig:freq-did)). 
 
 
```{r freq-did, fig.cap="Frequentist version of core difference-in-difference model, with standard errors adjusted by Newey-West correction to reflect clustering in space and time. Points are mean estimate, error bars 95% confidence interval."}

did_dat  <- model_runs$did_fit[[which(model_runs$data_source == "pisco" & model_runs$var_names == "pisco_a" & model_runs$data_to_use == "all")]]$did_data
  

did_reg <-
  lm(
    log(total_biomass_density) ~ targeted * year_bins + site_side - 1 + var_tex + var_tex_2
...
a = vcovPL(did_reg, cluster = ~  region +year,lag = "NW1994")


freq_did <- broom.mixed::tidy(did_reg) %>% 
  mutate(se = sqrt(diag(a))) %>% 
  mutate(effect = exp(estimate + se^2 / 2) - 1)

freq_did %>% 
  filter(str_detect(term, "targeted:")) %>% 
  mutate(term = str_remove_all(term, "targeted:year_bins")) %>% 
...
targeted_index <- base_run$did_fit[[1]]$did_data %>%
  group_by(year, targeted) %>%
  summarise(mtbd = mean(total_biomass_density)) %>%
  group_by(targeted) %>%
  filter(targeted == 1) %>% 
  mutate(classcode = "targeted")


nontargeted_index <- abundance_data$data[[which(abundance_data$data_source == "pisco")]] %>% 
  group_by(year, targeted, classcode, site_side) %>% 
  summarise(tb = sum(total_biomass_g)) %>% 
  group_by(year, targeted, classcode) %>% 
  summarise(mtbd = mean(tb)) %>% 
  filter(targeted == 0) %>% 
  ungroup() %>% 
  select(colnames(targeted_index))
...
synth_data <- targeted_index %>% 
  ...
  group_by(classcode) %>% 
  mutate(scaled_mtbd = as.numeric(scale(mtbd))) %>% 
  ungroup() %>% 
  as.data.frame()  %>% 
  select(-rolling_mean)
...
synth_data %>% 
  group_by(numeric_classcode, year) %>% 
  count()
...
```{r  total-synth, fig.cap = "Difference in centered and scaled standardized targeted abundance and synthetic standardized targeted abundance", eval=TRUE}
total_gap_plot
```
...
```{r synth-classcode-fits, fig.cap="Synthetic control gaps for each targeted species", eval=TRUE}
synth_classcode_fits_plot + 
  theme_minimal()
```
