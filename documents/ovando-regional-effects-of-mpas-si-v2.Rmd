...
```{r mean_test_plot, fig.cap = "Posterior predictive sum (distribution) and empirical sum of total biomass density. Posterior predictive sum adequately recovers empirical sum."}
...
```{r no-bin-plot, fig.cap="For ease of interpretation and model convergence, our default model estimates MPA effects in three-year bins. For this run we estimate the effect annually, relative to the year 2000. Results show the same overall trend and magnitude reported in our base model.", results = "hide", eval = FALSE}
no_bin <- model_runs %>%
  slice(1) %>%
  left_join(abundance_data, by = "data_source") %>%
  mutate(
