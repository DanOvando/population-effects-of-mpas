...
#                   numeric_species_key = numeric_species_key)
  ...
  standard_non_nested <- seen_cdata$x_non_nested %>%
  ...
  standard_non_nested <-
    standard_non_nested[rep(1, n_distinct(seen_data$factor_year)), ]

  standard_non_nested <-
    standard_non_nested[colnames(seen_cdata$x_non_nested)]


  standard_year_species <- seen_cdata$x_year_species %>%
    gather(variable, value) %>%
    group_by(variable) %>%
    summarise(mean_value = mean(value, na.rm = T)) %>%
    spread(variable, mean_value)
  ...
  standard_year_species <-
    standard_year_species[rep(1, n_distinct(seen_data$factor_year)), ]

  standard_year_species <-
    standard_year_species[colnames(seen_cdata$x_year_species)]


  standard_region_cluster <- seen_cdata$x_region_cluster %>%
    gather(variable, value) %>%
    group_by(variable) %>%
    summarise(mean_value = mean(value, na.rm = T)) %>%
    spread(variable, mean_value)
  ...
  standard_region_cluster <-
    standard_region_cluster[rep(1, n_distinct(seen_data$factor_year)), ]

  standard_region_cluster <-
    standard_region_cluster[colnames(seen_cdata$x_region_cluster)]


  standard_did_with_mpa <-
    data_frame(year = unique(seen_data$factor_year)) %>%
  ...
  ahnold_data <- list(
    ...
    region_cluster_index = seen_cdata$region_cluster_index,
  ...
  if (use_tmb == T) {
    ...
    {
      randos <- c(
        "seen_year_species_betas", 'seen_region_cluster_betas')  }
