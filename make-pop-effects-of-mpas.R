...
# there are ci simulations without any years_protected == 0
  ...
  pop_size_plot <- outcomes %>%
    ...
    scale_y_continuous(labels = percent, name = "Pop. Effect", expand = expansion(0,0)) +
    theme(legend.position = "top")
...
if (knit_paper == TRUE){
  ...
  rmarkdown::render(here::here("documents","ovando-et-al-pop-effects-of-mpas.Rmd"), params = list(run_name = run_name))

  rmarkdown::render(here::here("documents","ovando-et-al-pop-effects-of-mpas-si.Rmd"), params = list(run_name = run_name))
