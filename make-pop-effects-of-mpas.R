...
run_did <-
  FALSE # run difference in difference on data from the CINMS
...
if (run_did == TRUE) {
  ...
  length_data$yoy <-
    str_detect(tolower(length_data$classcode), "_yoy")
  
  length_data$classcode <-
    str_remove_all(length_data$classcode, "_yoy")
  
  # flip situations where min_tl is greater than max_tl
  
  flipped <-
    length_data$max_tl < length_data$min_tl &
    !is.na(length_data$max_tl) & !is.na(length_data$min_tl)
  
  length_data[flipped, c("min_tl", "max_tl")] <-
    length_data[flipped, c("max_tl", "min_tl")]
  ...
  life_history_data <- life_history_data %>%
    left_join(caselle_fished_species,
              by = c('commonname' = 'mod_commonname')) %>%
  ...
  if (get_cdfw_catches) {
    ...
    names <-
      tibble(com_name = commnames,
             sci_name =  map_chr(sci_names,  ~ .x[1])) %>%
  ...
  site_data <-
    read_csv(here::here(data_dir, 'Final_Site_Table_UCSB.csv')) %>%
    magrittr::set_colnames(., tolower(colnames(.))) %>%
    select(
      site,
      side,
  ...
  kelp_conn <-
    ncdf4::nc_open(here::here("data", "LandsatKelpBiomass_2017.nc"))
  ...
  kelp <- ncdf4::ncvar_get(kelp_conn, varid = c("biomass")) %>%
    as_data_frame() %>%
    mutate(
      lat = ncdf4::ncvar_get(kelp_conn, varid = c("lat")),
  ...
  time <-
    data_frame(
      year = ncdf4::ncvar_get(kelp_conn,
                              varid = c("year")),
  ...
  kelp <- kelp %>%
    ...
    select(-tdex) %>%
    mutate(
      rounded_lat = plyr::round_any(lat, 0.1),
  ...
  if (!file.exists(here::here("data", "SBCMBON_integrated_fish_20181129.csv"))) {
    ...
    download.file(
      "https://pasta.lternet.edu/package/data/eml/edi/5/3/4b1aed53da2cdbf25f9f9001bd74ace8",
  ...
  if (!file.exists(here::here("data", "SBCMBON_site_geolocation_20170523.csv"))) {
    ...
    download.file(
      "https://pasta.lternet.edu/package/data/eml/edi/5/3/f9eee1db42cd598cfb7b2578a35662fa",
  ...
  density_data <-
    read_csv(here::here(data_dir, "ci_reserve_data_final3 txt.csv")) %>%
    magrittr::set_colnames(., tolower(colnames(.))) %>%
  ...
  site_coords <- density_data %>%
    group_by(site, side) %>%
    summarise(
      latitude = mean(lon.wgs84, na.rm = T),
  ...
  if (file.exists(here("data", "processed-pisco-data.rds")) == F |
    ...
    for (i in 1:length(classcodes)) {
      ...
      if (any((colnames(temp_life_history) == colnames(pisco_data[1, life_history_vars])) == F)) {
        stop()
      }
    ...
    pisco_data <-
      read_rds(file.path(run_dir, "processed-pisco-data.rds"))
  ...
  abundance_data <- pisco_data %>%
    left_join(site_data, by = c("site", "side")) %>%
    left_join(lag_catches %>% select(year, classcode, lag_catch),
              by = c('year', 'classcode')) %>%
    mutate(data_source = 'pisco') %>%
    group_by(data_source, classcode) %>%
    nest() %>%
    bind_rows(
      kfm_data %>%  mutate(data_source = 'kfm') %>%
        left_join(site_data, by = c("site", "side")) %>%
        left_join(
          lag_catches %>% select(year, classcode, lag_catch),
          by = c('year', 'classcode')
        ) %>%
        mutate(year_month = year + (month / 12 - .1),
               site_side = site_id) %>%
    ...
    mutate(data = map(data, ~ left_join(
      .x, rolling_mean_temperatures, by = c("year")
    ))) %>%
    mutate(data = map(data, ~ mutate(
      .x, temp_deviation = abs(rolling_mean - temperature)
    ))) %>%
    mutate(data = map(data, ~ mutate(.x, month = as.numeric(
      as.character(factor_month)
    )))) %>%
    mutate(data = map(data, ~ left_join(.x, enso, by = c('year', 'month')))) %>%
    mutate(data = map(data, ~ left_join(.x, pdo, by = c('year', 'month')))) %>%
    mutate(data = map(data, ~ mutate(.x, lag_catch = ifelse(
      is.na(lag_catch), 0, lag_catch
    )))) %>%
    mutate(
      data = map2(
        data,
  ...
  pisco_cols <-
    abundance_data$data[abundance_data$data_source == "pisco"][1][[1]] %>% colnames()
  ...
  abundance_data <- abundance_data %>%
    mutate(data = map2(data, list(kelp_model),  ~ mutate(
      .x, interp_kelp = predict(
        .y,
        newdata = .x %>% mutate(quarter = (month / 3) %>% ceiling()) %>%  select(year, quarter, rounded_lat, rounded_lon)
      )
    )))
  
  save(file = file.path(run_dir, "abundance_data.Rdata"),
       abundance_data)
  ...
  model_runs <- cross_df(
    ...
    filter(!(data_source == "kfm" &
               str_detect(var_names, "pisco"))) %>%
    filter(!(data_source == "kfm" &
               (
                 data_to_use %in% c("mpa_only", "fished_only")
               )))
  ...
  if (run_tmb == T) {
    ...
    fits <- foreach::foreach(i = 1:nrow(tmb_runs)) %dopar% {
      ...
      fits <- sfz(
        ...
        fixed_did = FALSE,
        non_nested_did_variables = c("temp", "kelp", "lag_catch"),
        bin_years = bin_years
      )
  ...
  plot_did <- function(x) {
    mpa_effect_plot <-  x$did_results %>%
      ...
      tidybayes::stat_halfeye(alpha = 0.7,
                              .width = c(0.5, 0.95)) +
      scale_y_continuous(labels = percent, name = "Estimated Pop. Effect") +
...
if (simulate_mpas == TRUE) {
  ...
  fitted_data <- fitted_data %>%
    left_join(life_history_data %>% select(classcode, taxa) %>% unique(),
              by = "classcode")
  ...
  if (run_experiments == T) {
    if (create_grid == TRUE) {
      ...
      fish_worked <-
        map(sim_grid$fish, "error") %>% map_lgl(is_null)
      sim_grid <- sim_grid %>%
        filter(fish_worked) %>%
      ...
      tuning_worked <-
        map(tuned_results, "error") %>% map_lgl(is.null)
    ...
    size_grid <- tibble(obs = obs) %>%
      mutate(size = map_dbl(obs, ~ as.numeric(object.size(get(
        .x
      ))))) %>%
      arrange(desc(size))
  ...
  loadfoo <-
    function(experiment, experiment_dir, output = "mpa-effect") {
      ex <-
        readRDS(file.path(
          experiment_dir,
          glue::glue("experiment_{experiment}.rds")
        ))
...
if (simulate_channel_islands == TRUE) {
  ...
  if (run_experiments == T) {
    if (create_grid == TRUE) {
      ...
      pisco_genus <-
        unique(pisco_data$genus[pisco_data$targeted == 1])
      
      ci_fish <- stringr::str_trim(rfishbase::taxonomy())
      
      which_ci_fish <-
        stringr::str_split(ci_fish, pattern  = ' ', simplify = TRUE)
      ...
      sim_grid <-
        tibble(
          ...
          fleet_model = sample(c("open-access", "constant-effort"),
                               samps,
                               replace = T),
          effort_allocation = sample(
            c("profit-gravity", "simple", "gravity"),
      ...
      write_rds(sim_grid,
                file.path(run_dir, "initial_ci_sim_grid.rds"))
      ...
      tuned_fisheries <-
        foreach::foreach(
          ...
          filename <-
            glue::glue("ci_fishery_{sim_grid$experiment[i]}.rds")
      ...
      loadfoo <-
        function(fishery, experiment_dir) {
          ex <-
            readRDS(file.path(
              experiment_dir,
              glue::glue("ci_fishery_{fishery}.rds")
            ))
        }
      
      
      tuned_results <-
        map(sim_grid$experiment, safely(loadfoo), experiment_dir = experiment_dir)
    ...
    mpa_experiments <-
      foreach::foreach(
        ...
        filename <-
          glue::glue("ci_experiment_{sim_grid$experiment[i]}.rds")
        #
        # out <- results
  ...
  loadfoo <-
    function(experiment, experiment_dir, output = "mpa-effect") {
      ex <-
        readRDS(file.path(
          experiment_dir,
          glue::glue("ci_experiment_{experiment}.rds")
        ))
...
if (validate_mpas == TRUE) {
  ...
  validate_foo <- function(sigma_r,
    ...
    enviro <-
      rep(c(rep(0, 5), rep(env_strength, 5)), (sim_years + burn_years) / 10)
  ...
  valplot <- function(x, year_mpa = 50) {
    did_plot <- x$did_values %>%
      ...
      geom_line(aes(years_protected, mpa_effect))
  }
  
  valgrid <- valgrid %>%
    mutate(valplot = map(validation, valplot))
  
  write_rds(valgrid %>% select(-valplot),
            file.path(run_dir, "valgrid.rds"))
...
write(
  glue::glue("Figures generated on {Sys.Date()} using results {run_name}"),
...
ca_mpas <-
  sf::st_read(here::here("data", "MPA_CA_Existing_160301")) %>%
...
processed_grid$adult_movement <-
  (2 * processed_grid$adult_movement) / num_patches
...
bad_sims <- processed_grid %>%
  ...
  summarise(bad = any(depletion > 0.95 |
                        abs(pop_effect) > 1e-3 |
                        no_mpa)) %>% #removed runs that either crashed pre-mpa, or had MPA effects before MPA (improper burn in)
  filter(bad == TRUE)
...
write_rds(processed_grid,
          file.path(run_dir, "filtered_processed_grid.rds"))
...
simmed_fish_life <- simmed_fish_life %>%
  # slice(1:10) %>%
  dplyr::mutate(life_traits = future_pmap(list(Genus = genus, Species = species),
                                          sq,
                                          .progress = TRUE))
...
outcomes <- processed_grid %>%
  select(-mpa_size) %>%
  left_join(simmed_fish_life %>% select(taxa, m),
            by = c("scientific_name" = "taxa")) %>%
...
density_ratios <- processed_grid %>%
  ...
  unnest(cols = c(density_ratio, mpa_effect),
         names_repair = "unique") %>%
...
effect_example_plot <- compare %>%
  ...
  ggplot() +
  geom_ribbon(
    data = ribbon %>% filter(mpa_size == 0.25),
    aes(
      years_protected,
      ymin = `no-mpa`,
      ymax = `with-mpa`,
  ...
  scale_color_aaas(labels = c("Without MPAs", "With MPAs"),
                   name  = '') +
...
site_data <-
  read_csv(here::here("data", 'Final_Site_Table_UCSB.csv')) %>%
  magrittr::set_colnames(., tolower(colnames(.))) %>%
  select(
    site,
    side,
...
consistent_sites <- pisco_abundance_data %>%
  ...
  ungroup() %>%
  filter(has_all) %>%
  left_join(site_data %>% unite("site_side", site, side, sep  = "-"),
            by = c("site_side")) %>%
...
targ_rr_coefs <-
  tidybayes::spread_draws(targ_rr_fit, `(Intercept)`, b[year, mpa]) %>%
  ungroup() %>%
...
nontarg_rr_coefs <-
  tidybayes::spread_draws(nontarg_rr_fit, `(Intercept)`, b[year, mpa]) %>%
  ungroup() %>%
  filter(!str_detect(mpa, "region")) %>%
  mutate(year = as.integer(str_remove_all(year, "fyear")),
         mpa = as.integer(str_remove_all(mpa, "eventual_mpa:"))) %>%
...
median_effect <-
  median(short_term$mpa_effect[short_term$year == max(short_term$year)])

median_pop_effect <-
  median(short_term$pop_effect[short_term$year == max(short_term$year)])

median_fish_effect <-
  median(fishery_short_term$mpa_effect[short_term$year == max(short_term$year)], na.rm = TRUE)

median_fish_effect <-
  median(fishery_short_term$msy_effect[short_term$year == max(short_term$year)], na.rm = TRUE)


pos_runs <-
  round(mean(short_term$pop_effect[short_term$year == max(short_term$year)] > 0), 2)

pos_fish_runs <-
  round(mean(fishery_short_term$msy_effect[fishery_short_term$year == max(fishery_short_term$year)] > 0, na.rm = TRUE), 2)

pos_fish_effect <-
  round(mean(short_term$f[short_term$year == max(short_term$year)] > 0), 2)

small_effect <-
  round(100 * median(short_term$mpa_effect[short_term$mpa_size <= 0.25 &
                                             short_term$depletion <= 0.5]))

min_small_effect <-
  round(100 * min(short_term$mpa_effect[short_term$mpa_size <= 0.25 &
                                          short_term$depletion <= 0.5]))

max_small_effect <-
  round(100 * max(short_term$mpa_effect[short_term$mpa_size <= 0.25 &
                                          short_term$depletion <= 0.5]))

medium_effect <-
  round(100 * median(short_term$mpa_effect[short_term$depletion > 0.5 &
                                             short_term$depletion <= 0.75]))
...
fleet_importance_plot <- outcomes %>%
  ...
  scale_fill_npg() +
  labs(y = "# of Sims") +
  scale_x_percent(name = "Pop. Level Conservation Effect")
...
cc_importance_plot <- outcomes %>%
  group_by(experiment) %>%
  filter(
    years_protected == max(years_protected),
    fleet_model == "constant-catch",
    mpa_effect < 0
  ) %>%
...
pop_facet_effect_plot <- outcomes %>%
  ...
  scale_y_percent(name = "Pop. Effect")
...
eq_pop_depletion_and_size_plot <- outcomes %>%
  ...
  group_by(experiment) %>%
  mutate(
    depletion = plyr::round_any(depletion[years_protected == max(years_protected)], .05),
...
eq_pop_f_and_size_plot <- outcomes %>%
  ...
  group_by(experiment) %>%
  mutate(
    f = plyr::round_any(f_v_m[years_protected == 0], .1),
    mpa_size = plyr::round_any(mpa_size, .05)
  ) %>%
  filter(years_protected == max(years_protected)) %>%
  group_by(f, mpa_size) %>%
...
catch_facet_effect_plot <- fishery_outcomes %>%
  ...
  ggplot() +
  geom_bin2d(aes(value, fishery_effect),
             binwidth = c(.05, .05),
             show.legend = TRUE) +
  scale_fill_viridis(option = "A",
                     guide = gc,
...
catch_fishery_combo_plot <-
  (catch_depletion_and_size_plot + labs(title = "A")) + ((catch_size_plot + labs(title = "B")) / catch_depletion_plot)  + plot_layout(widths = c(1.5, 1)) &
  theme(
...
unbiased_dr <- density_ratios %>%
  ...
  labs(
    x = "Percent of Unfished Biomass",
    y = "Percent Difference in Inside/Outside MPA Densities",
    caption = "Density ratios calculated as density inside MPAs relative to density of identical scenario without MPAs",
...
biased_dr <- density_ratios %>%
  ...
  labs(
    x = "Percent of Unfished Biomass",
    y = "Percent Difference in Inside/Outside MPA Densities",
    caption = "Density ratios calculated as density inside MPAs relative to density outside MPAs, weighted by distance from MPA border",
...
dat <- dat %>%
  ...
  group_by(region) %>%
  mutate_at(vars(carnivore:planktivore), funs(norm = (. - mean(.)) / sd(.))) ->
  dat
...
group_trends_plot <- dat %>%
  ...
  scale_color_locuszoom(
    name = "Trophic Troup",
    labels = c("carnivore", "herbivore", "piscivore", "planktivore")
  ) +
...
datnorm <-
  dat %>% select(region, year, contains('norm')) %>% ungroup()
...
segs <-
  datnorm %>% mutate(ind = row_number()) %>% group_by(region) %>% summarise(first =
                                                                              first(ind), last = last(ind)) %>%
  select(-region)
...
regions.combined.simp.list <- map(var_names, function(x) {
  temp <-
    datnorm %>% ungroup() %>% select(matches(x)) %>% as.data.frame()
  out <-
    simplex(
      as.numeric(temp[, 1]),
      E = 1:10,
...
temp <-
  ccm(
    ...
    pred = as.matrix(segs),
    E = tempE,
    lib_column = 'carnivore_norm',
    target_column = 'herbivore_norm',
    lib_sizes = c(10, 25, 50, 75),
    num_samples = 100,
    replace = T,
    silent = T,
    RNGseed = 41389
...
c_xmap_h <- temp %>%
  ...
  labs(x = "",
       y = expression(paste(rho, " (predictive skill)")),
       title = "Carnivores on Herbivores")
...
temp <-
  ccm(
    ...
    pred = as.matrix(segs),
    E = tempE,
    lib_column = 'herbivore_norm',
    target_column = 'carnivore_norm',
    lib_sizes = c(10, 25, 50, 75),
    num_samples = 100,
    replace = T,
    silent = T,
...
h_xmap_c <- temp %>%
  ...
  labs(x = "Library Size",
       y = expression(paste(rho, " (predictive skill)")),
       title = "Herbivores on Carnivores")
...
doh <- raw %>%
  ...
  ggplot() +
  geom_col(aes(patch, `with-mpa`, fill = mpa),
           width = 1) +
...
simple_cs_plot <- raw_sum %>%
  ...
  labs(x = "Year") +
  scale_color_npg(labels = c("Without MPAs", "With MPAs"),
                  name = '')
...
channel_islands <-
  sf::st_read(here::here("data", "cinms_py2", "cinms_py.shp"))
...
implications %>%
  ...
  ggridges::geom_density_ridges(alpha = 0.75, color = "transparent") +
  scale_x_continuous(name = "Simulated Pop. Effect", labels = percent) +
  scale_y_continuous(name = element_blank()) +
  labs(title = "Targeted")
...
sim_mpa_effects <- outcomes %>%
  left_join(simmed_fish_life %>% select(-m),
            by = c("scientific_name" = "taxa")) %>%
  filter(set == "ci",
         final_depletion > 0,
         f_v_m > .5) %>%
  ...
  filter(year >= 2003,!is.na(binned_year)) %>%
  mutate(mpa_effect = pmin(mpa_effect, 3))

ylabs <-
  c(paste0(seq(-50, 250, by = 50), "%"), expression("" >= "300%"))
...
savefoo <- function(fig,
                    ...
                    fig_height = 5) {
  ggsave(
    filename =  file.path(fig_dir, paste(fig, extension, sep = '.')),
    plot  = get(fig),
    width = fig_width,
    height = fig_height,
    device = cairo_pdf)
...
if (knit_paper == TRUE) {
  rmarkdown::render(
    here::here("documents", "ovando-et-al-pop-effects-of-mpas.Rmd"),
    params = list(run_name = run_name)
  )
  
  rmarkdown::render(
    here::here("documents", "ovando-et-al-pop-effects-of-mpas-si.Rmd"),
    params = list(run_name = run_name)
  )
